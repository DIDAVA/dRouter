...
class dRouter {
    ...
    public function all($pattern, $fn) { $this->match('GET|POST|PUT|DELETE|OPTIONS|PATCH|HEAD', $pattern, $fn); }
    ...
    public function run($callback = null) {
      ...
      if (isset($this->beforeRoutes[$this->requestedMethod])) $this->handle($this->beforeRoutes[$this->requestedMethod]);
      $numHandled = 0;
      if (isset($this->afterRoutes[$this->requestedMethod])) $numHandled = $this->handle($this->afterRoutes[$this->requestedMethod], true);
      if ($numHandled === 0) $this->trigger404();
      else if ($callback && is_callable($callback)) $callback();
    ...
    public function trigger404(){
      if ($this->notFoundCallback) $this->invoke($this->notFoundCallback);
    ...
    private function invoke($fn, $params = []) {
      if (is_callable($fn)) call_user_func_array($fn, $params);
      elseif (stripos($fn, '@') !== false) {
        list($controller, $method) = explode('@', $fn);
        if ($this->getNamespace() !== '') $controller = $this->getNamespace() . '\\' . $controller;
        try {
